...
export const createNotification = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    if (recipientType === 'all') {
      ...
      recipients = req.body.recipients.map((id: string) => ({ _id: id }));
    } else {
...
export const getNotificationById = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    if (req.user?.role === 'student' && notification.userId.toString() !== userId) {
...
export const sendBulkNotifications = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    for (let i = 0; i < notifications.length; i++) {
      try {
        ...
        if (!title || !message || !type || !category) {
          errors.push(`Row ${i + 1}: Title, message, type, and category are required`);
          continue;
        }
        ...
        if (recipientType === 'all') {
          ...
          recipients = notifications[i].recipients.map((id: string) => ({ _id: id }));
        } else {
