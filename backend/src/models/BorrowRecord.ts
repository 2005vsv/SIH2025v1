...
export interface IBorrowRecord extends Document {
  ...
  canRenew: boolean;
  // Methods
  renew(days?: number): boolean;
  returnBook(condition?: string): void;
}
...
const BorrowRecordSchema = new Schema<IBorrowRecord>({
  ...
  dueDate: {
    ...
    validate: {
      validator: function(this: IBorrowRecord, value: Date) {
...
BorrowRecordSchema.virtual('daysBorrowed').get(function(this: IBorrowRecord) {
...
BorrowRecordSchema.virtual('daysOverdue').get(function(this: IBorrowRecord) {
  const now = new Date();
...
BorrowRecordSchema.virtual('canRenew').get(function(this: IBorrowRecord) {
...
BorrowRecordSchema.methods.renew = function(this: IBorrowRecord, days: number = 14): boolean {
...
BorrowRecordSchema.methods.returnBook = function(this: IBorrowRecord, condition: string = 'good'): void {
  this.returnDate = new Date();
  this.status = 'returned';
  this.condition = condition as any;
};
...
BorrowRecordSchema.statics.getUserStats = function(userId: mongoose.Types.ObjectId) {
...
BorrowRecordSchema.statics.getBookStats = function(bookId: mongoose.Types.ObjectId) {
  return this.aggregate([
    { $match: { bookId } },
    {
      $group: {
